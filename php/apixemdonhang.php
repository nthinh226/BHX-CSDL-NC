...
switch ($event) {
    ...
    case "getALLDonHangTrongNgay":
        ...
        $t1 = "23:59:59";
        $to = date('Y-m-d') . " " . $t1;
        $sql = mysqli_query($conn, "select v.sodh, v.makh, v.hotenkh, v.email, v.sdt, v.diachigiaohang, v.ngaydh, v.tongtien, v.trangthaidh from view_donhang v where v.trangthaidh = 0 and v.ngaydh between '" . $from . "' and '" . $to . "' and (v.hotenkh like '%" . $search . "%' or v.sodh like '%" . $search . "%') order by v.sodh asc ");
    ...
    case "getALLSPDonHang":
        $mang = array();
        $sodh = $_POST['sodh'];
        $sql = mysqli_query($conn, "select c.masp, s.tensp, c.soluong, c.giatien, c.giamgia, thanhtien(c.soluong,c.giatien,c.giamgia) as thanhtien, v.tongtien from chitietdh c, sanpham s, view_donhang v where c.sodh = v.sodh and c.masp = s.masp and c.sodh = '" . $sodh . "'");
        while ($rows = mysqli_fetch_array($sql)) {
    ...
    case "getALLDonHangDangXuLy":
        $mang = array();
        $search = $_POST['search']; //Tìm kiếm dữ liệu
        $sql = mysqli_query($conn, "select v.sodh, v.makh, v.hotenkh, v.email, v.sdt, v.diachigiaohang, v.ngaydh, v.tongtien, v.trangthaidh from view_donhang v where (v.trangthaidh = 0 or v.trangthaidh = 1) and (v.hotenkh like '%" . $search . "%' or v.sodh like '%" . $search . "%') order by v.sodh asc ");
    ...
    case "getALLDonHangHoanThanh":
        $mang = array();
        $search = $_POST['search']; //Tìm kiếm dữ liệu
        $sql = mysqli_query($conn, "select v.sodh, v.makh, v.hotenkh, v.email, v.sdt, v.diachigiaohang, v.ngaydh, v.tongtien, v.trangthaidh from view_donhang v where (v.trangthaidh = 2 or v.trangthaidh = 3) and (v.hotenkh like '%" . $search . "%' or v.sodh like '%" . $search . "%') order by v.sodh asc ");
